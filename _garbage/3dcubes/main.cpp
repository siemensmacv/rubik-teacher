...
int main(int argc, char **argv)
{
    ...
    buton->setText(QStringLiteral("Rotate counterclockwise"));

    verticalLayout->addWidget(buton);

    // Button 2
    QPushButton *buton2 = new QPushButton(widget);
    buton2->setText(QStringLiteral("Rotate clockwise"));

    verticalLayout->addWidget(buton2);

    // Rotation
    RotatingEventHandler *handler = new RotatingEventHandler(buton, buton2, cube2Controller);
    handler->addAnimation(cube1->m_cubeAnim);
    ...
    QObject::connect(buton, &QPushButton::clicked,
                     handler, &RotatingEventHandler::buttonPressed);

    QObject::connect(buton2, &QPushButton::clicked,
                     handler, &RotatingEventHandler::button2Pressed);
