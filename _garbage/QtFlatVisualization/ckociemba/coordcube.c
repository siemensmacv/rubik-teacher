...
char * join_path(const char *dir, const char *filename)
{
    ...
    size_t buffer_len = path_len + 32;
    char *fpath = calloc(path_len + 32, 1);
    strcpy_s(fpath, path_len + 32, dir);
    strcat_s(fpath, buffer_len, "/");
    strncat_s(fpath, buffer_len, filename, 30);
    return fpath;
}
...
int check_cached_table(const char* name, void* ptr, int len, const char *cache_dir)
{
    ...
    int res = 0;
    if (_access_s(fname, 4/*F_OK | R_OK*/) == 0) {
...
void dump_to_file(void* ptr, int len, const char* name, const char *cache_dir)
{
    return;
    int status;
    status = _mkdir(cache_dir/*, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH*/);
